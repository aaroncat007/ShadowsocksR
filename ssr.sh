...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		ss_ssr_determine
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR賬號 配置信息：" && echo
...
Set_config_port(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR賬號 端口"
...
Set_config_method(){
	echo -e "請選擇要設置的ShadowsocksR賬號 加密方式
...
Set_config_protocol(){
	echo -e "請選擇要設置的ShadowsocksR賬號 協議插件
 ...
 ${Tip} 如果使用 auth_chain_a 協議，請加密方式選擇 none，混淆隨意(建議 plain)" && echo
	read -e -p "(默認: 2. auth_sha1_v4):" ssr_protocol
...
Set_config_obfs(){
	echo -e "請選擇要設置的ShadowsocksR賬號 混淆插件
 ...
 ${Tip} 如果使用 ShadowsocksR 加速遊戲，請選擇 混淆兼容原版或 plain 混淆，然後客戶端選擇 plain，否則會增加延遲 !
...
Set_config_protocol_param(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR賬號 欲限制的設備數 (${Green_font_prefix} auth_* 系列協議 不兼容原版才有效 ${Font_color_suffix})"
...
Set_config_speed_limit_per_con(){
	while true
	do
	echo -e "請輸入要設置的每個端口 單線程 限速上限(單位：KB/S)"
...
Set_config_speed_limit_per_user(){
	...
	do
	echo
	echo -e "請輸入要設置的每個端口 總速度 限速上限(單位：KB/S)"
...
Download_SSR(){
	...
	mkdir ${config_folder}
	[[ ! -e ${config_folder} ]] && echo -e "${Error} ShadowsocksR配置文件的文件夾 建立失敗 !" && exit 1
...
debian_View_user_connection_info(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		user_list_all="端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 鏈接IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 當前鏈接IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
		user_IP=""
		echo -e "當前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 鏈接IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix}"
		...
		do
			...
			user_list_all=${user_list_all}"端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 鏈接IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 當前鏈接IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
			user_IP=""
		done
		echo -e "當前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 用戶總數: ${Green_background_prefix} "${user_total}" ${Font_color_suffix} 鏈接IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix} "
		echo -e "${user_list_all}"
	fi
}
...
Modify_multi_port_user(){
	...
	read -e -p "(默認: 取消):" modify_user_port
...
Port_mode_switching(){
	SSR_installation_status
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e "	當前模式: ${Green_font_prefix}單端口${Font_color_suffix}" && echo
		echo -e "確定要切換為 多端口模式？[y/N]"
		read -e -p "(默認: n):" mode_yn
		[[ -z ${mode_yn} ]] && mode_yn="n"
		if [[ ${mode_yn} == [Yy] ]]; then
			port=`${jq_file} '.server_port' ${config_user_file}`
			Set_config_all
		...
		echo && echo -e "	當前模式: ${Green_font_prefix}多端口${Font_color_suffix}" && echo
...
View_Log(){
	...
	echo && echo -e "${Tip} 按 ${Red_font_prefix}Ctrl+C${Font_color_suffix} 終止查看日志" && echo -e "如果需要查看完整日志內容，請用 ${Red_font_prefix}cat ${ssr_log_file}${Font_color_suffix} 命令。" && echo
	tail -f ${ssr_log_file}
}
# 銳速
...
Other_functions(){
  ...
  註意： 銳速/LotServer/BBR 不支持 OpenVZ！
  註意： 銳速/LotServer/BBR 不能共存！
————————————
  ...
  ${Green_font_prefix}6.${Font_color_suffix} 切換 ShadowsocksR日志輸出模式
  ——說明：SSR默認只輸出錯誤日志，此項可切換為輸出詳細的訪問日志" && echo
...
Set_config_connect_verbose_info(){
	...
	if [[ ${connect_verbose_info} = "0" ]]; then
		echo && echo -e "當前日志模式: ${Green_font_prefix}簡單模式（只輸出錯誤日志）${Font_color_suffix}" && echo
		echo -e "確定要切換為 ${Green_font_prefix}詳細模式（輸出詳細連接日志+錯誤日志）${Font_color_suffix}？[y/N]"
		...
		echo && echo -e "當前日志模式: ${Green_font_prefix}詳細模式（輸出詳細連接日志+錯誤日志）${Font_color_suffix}" && echo
		echo -e "確定要切換為 ${Green_font_prefix}簡單模式（只輸出錯誤日志）${Font_color_suffix}？[y/N]"
		read -e -p "(默認: n):" connect_verbose_info_ny
		[[ -z "${connect_verbose_info_ny}" ]] && connect_verbose_info_ny="n"
		if [[ ${connect_verbose_info_ny} == [Yy] ]]; then
...
menu_status(){
	if [[ -e ${config_user_file} ]]; then
		...
		now_mode=$(cat "${config_user_file}"|grep '"port_password"')
		if [[ -z "${now_mode}" ]]; then
			echo -e " 當前模式: ${Green_font_prefix}單端口${Font_color_suffix}"
		else
			echo -e " 當前模式: ${Green_font_prefix}多端口${Font_color_suffix}"
		fi
	else
